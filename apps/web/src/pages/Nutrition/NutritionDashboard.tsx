...
function InlineSearch({ date, meal, onClose }: InlineSearchProps) {
    ...
    <div className="border-t" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
      ...
      <div className="flex items-center gap-2 px-4 py-3">
        <div className="relative flex-1">
          ...
          <input ref={inputRef} className="input pl-9 pr-8 text-sm" placeholder="Gıda ara... (tavuk, yulaf, elma)"
        ...
        <button onClick={() => setShowCustom(!showCustom)}
          className="text-xs px-2.5 py-1.5 rounded-lg font-medium whitespace-nowrap transition-all shrink-0"
          style={{ background: showCustom ? 'rgba(249,115,22,0.15)' : 'rgba(255,255,255,0.06)', color: showCustom ? '#fb923c' : '#9ca3af' }}>
          + Özel
        </button>
        <button onClick={onClose} className="text-gray-500 hover:text-white transition-colors shrink-0 p-1">
      ...
      {/* Custom food form */}
      {showCustom && (
        <div className="px-4 pb-3 border-t" style={{ borderColor: 'rgba(255,255,255,0.06)', background: 'rgba(249,115,22,0.03)' }}>
          <p className="text-xs font-semibold text-orange-400 mt-3 mb-2">Özel Gıda Oluştur</p>
      ...
      {/* Results */}
      {q.length >= 2 && (
        <div className="max-h-56 overflow-y-auto px-4 pb-3 space-y-1.5">
          {results.length === 0 && !isFetching && (
            <div className="text-center py-6">
              <p className="text-sm text-gray-500">"{q}" için sonuç bulunamadı</p>
              <button onClick={() => setShowCustom(true)} className="mt-1 text-xs text-orange-400 hover:underline">
                Özel gıda oluştur →
              </button>
            </div>
          )}
          {results.map((food) => (
            <div key={food.id} className="flex items-center gap-2 px-3 py-2.5 rounded-xl"
              ...
              <input
                ...
                className="w-14 text-center text-sm rounded-lg py-1 bg-white/5 border border-white/10 text-white"
              />
              <button onClick={() => addFood(food.id)} disabled={addLogMutation.isPending}
                ...
                <svg viewBox="0 0 24 24" fill="none" stroke={cfg.color} strokeWidth={2.5} className="w-4 h-4">
      ...
      {q.length < 2 && !showCustom && (
        <p className="text-xs text-center text-gray-700 pb-3">En az 2 karakter gir</p>
      )}
    </div>
  );
}
...
export default function NutritionDashboard() {
  ...
  const [activeSearch, setActiveSearch] = useState<MealKey | null>(null);
  ...
  function toggleSearch(meal: MealKey) {
    setActiveSearch((prev) => (prev === meal ? null : meal));
  }
    ...
    <div className="space-y-5 max-w-2xl">
      ...
      <div className="space-y-3">
        {(Object.entries(MEAL_CFG) as [MealKey, typeof MEAL_CFG[MealKey]][]).map(([mealKey, cfg]) => {
          ...
          const isOpen = activeSearch === mealKey;
          ...
          return (
            <div key={mealKey} className="card overflow-hidden">
              ...
              <div className="flex items-center justify-between px-4 py-3">
                ...
                <button onClick={() => toggleSearch(mealKey)}
              ...
              {/* Food items */}
              {logs.length > 0 && (
                <div className="divide-y" style={{ borderColor: 'rgba(255,255,255,0.04)', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                  {logs.map((log) => (
              ...
              {/* Inline search */}
              {isOpen && (
                <InlineSearch date={date} meal={mealKey} onClose={() => setActiveSearch(null)} />
              )}
            </div>
          );
        })}
      ...
      {!isLoading && (!summary?.logs || summary.logs.length === 0) && (
        <div className="text-center py-8 text-gray-600 text-sm">
          {isToday ? 'Bugün henüz öğün eklenmedi · Öğünlerden "+ Ekle" butonuna tıkla' : 'Bu gün için kayıt yok'}
        </div>
      )}
